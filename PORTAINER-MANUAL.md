...
## Docker Compose Content for Portainer (Standalone Version)

**Use this version - it creates all configuration files automatically:**
  ...
  coturn:
    ...
    command: >
      sh -c "
        ...
        echo 'realm=turn.$${SERVER_NAME}' >> /etc/coturn/turnserver.conf &&
        echo 'server-name=turn.$${SERVER_NAME}' >> /etc/coturn/turnserver.conf &&
        ...
        echo 'static-auth-secret=$${TURN_SECRET}' >> /etc/coturn/turnserver.conf &&
